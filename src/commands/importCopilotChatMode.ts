...
import {
  ...
  LLM_TOOLS_MODULE_NAME as LLM_TOOLS_MODULE_NAME, OASFLLMToolsModule,
...
function oasfToChatMode(oasf: OASFRecord): VscodeCopilotChatMode | undefined {
  ...
  if (!llmToolsModule) {
    console.log("LLM Tools module not found");
    return undefined;
  }
  ...
  if (!promptModule) {
    console.log("Required prompt module not found");
    return undefined;
  }
  ...
  if (modelName) {
    ...
    if (!modelModule) {
      console.log("Model modulenot found");
      return undefined;
    }
    model = modelModule.data.models.find(m => m.model === modelName);
    if (!model) {
      console.log("Model not found in the model module");
      return undefined;
    }
  }
  ...
  if (llmToolsModule.data.mcp_server_tools && llmToolsModule.data.mcp_server_tools.length > 0) {
    ...
    if (!mcpModule) {
      console.log("MCP module not found");
      return undefined;
    }
  }
